...
why the gopls application depends on the cmp package:
	...
	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps github.com/asahasrabuddhe/tools/gopls |
		digraph somepath github.com/asahasrabuddhe/tools/gopls github.com/google/go-cmp/cmp

Show which packages in x/tools depend, perhaps indirectly, on the callgraph package:

	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps github.com/asahasrabuddhe/tools/... |
		digraph reverse github.com/asahasrabuddhe/tools/go/callgraph
